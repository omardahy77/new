...
export const Home: React.FC = () => {
  ...
  // Show top 3 based on the filter
      ...
      <section className="relative z-10 section-padding bg-[#020305] border-t border-white/5">
           <div className="container-custom">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                 {[
                    { id: 'feat_analysis', icon: LineChart, defaultTitle: t('feat_analysis'), defaultDesc: t('feat_analysis_desc') },
                    { id: 'feat_risk', icon: Shield, defaultTitle: t('feat_risk'), defaultDesc: t('feat_risk_desc') },
                    { id: 'feat_psych', icon: Brain, defaultTitle: t('feat_psych'), defaultDesc: t('feat_psych_desc') },
                    { id: 'feat_community', icon: Users, defaultTitle: t('feat_community'), defaultDesc: t('feat_community_desc') }
                 ].map((feat) => (
                    <div key={feat.id} className="bg-[#05070E] border border-white/5 p-8 rounded-2xl hover:-translate-y-2 transition-all duration-300 text-center group hover:border-gold-500/20">
                        <div className="w-16 h-16 bg-[#020204] rounded-2xl flex items-center justify-center mx-auto mb-6 border border-white/5 group-hover:border-gold-500/20 transition-colors">
                           <feat.icon size={32} className="text-gold-500" />
                        </div>
                        <h3 className="text-xl font-black text-white mb-3">
                            {getContent(`${feat.id}_title`) || feat.defaultTitle}
                        </h3>
                        <p className="text-sm text-gray-500 leading-relaxed font-bold">
                           {getContent(`${feat.id}_desc`) || feat.defaultDesc}
                        </p>
                    </div>
                 ))}
              </div>
        ...
        <div className="container-custom">
          <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-4">
            <div>
              ...
              <p className="text-gray-500 text-lg font-bold">
                {user ? t('latest_courses_sub') : 'سجل الدخول لمشاهدة الكورسات المدفوعة'}
              </p>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredCourses.length === 0 ? (
                <div className="col-span-full text-center py-10 text-gray-500 bg-white/5 rounded-2xl border border-white/5">
                    <p className="font-bold">لا توجد كورسات متاحة للعرض حالياً.</p>
                    {!user && <p className="text-sm mt-2">سجل الدخول لرؤية الكورسات المدفوعة.</p>}
                </div>
            ) : (
      ...
      <section className="bg-[#020305] pt-24 pb-20 text-center relative z-10 border-t border-white/5">
         <div className="container-custom flex flex-col items-center">
            ...
            <div className="flex items-center justify-center gap-6">
                {links.facebook && (
                    <a href={links.facebook} target="_blank" rel="noreferrer" className="w-14 h-14 rounded-2xl bg-[#05070E] border border-white/10 flex items-center justify-center text-[#1877F2] hover:bg-[#1877F2] hover:text-white hover:border-[#1877F2] transition-all duration-300 shadow-lg hover:-translate-y-2 hover:shadow-[#1877F2]/40">
                        <Facebook size={28} />
                    </a>
                )}
                {links.instagram && (
                    <a href={links.instagram} target="_blank" rel="noreferrer" className="w-14 h-14 rounded-2xl bg-[#05070E] border border-white/10 flex items-center justify-center text-[#E4405F] hover:bg-[#E4405F] hover:text-white hover:border-[#E4405F] transition-all duration-300 shadow-lg hover:-translate-y-2 hover:shadow-[#E4405F]/40">
                        <Instagram size={28} />
                    </a>
                )}
                {links.telegram && (
                    <a href={links.telegram} target="_blank" rel="noreferrer" className="w-14 h-14 rounded-2xl bg-[#05070E] border border-white/10 flex items-center justify-center text-[#229ED9] hover:bg-[#229ED9] hover:text-white hover:border-[#229ED9] transition-all duration-300 shadow-lg hover:-translate-y-2 hover:shadow-[#229ED9]/40">
                        <Send size={28} />
                    </a>
                )}
            </div>
